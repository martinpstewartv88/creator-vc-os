-- Widen email-change + merge RPC gates: owner+admin → admin+support.
--
-- Rationale: the owner (Martin) doesn't operate the app day-to-day.
-- Support and admin staff handle customer email consolidation, and
-- support role already has full access to Customers + Tickets screens
-- (per the earlier lockdown migration). The audit table still records
-- the acting user's uid so accountability is preserved.
--
-- Only the auth gate lines change; the transactional body is identical
-- to the previous versions (20260703150000_customer_email_change_cascade
-- and 20260703170000_customer_merge_cascade). Do not overwrite the
-- satellite-table UPDATE list without preserving both.
...
begin
  ...
  if public.current_app_role() not in ('admin','support') then
    raise exception 'forbidden: admin or support role required' using errcode = '42501';
  end if;
